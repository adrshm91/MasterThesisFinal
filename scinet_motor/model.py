...
class Network(object):
    ...
    def train(self, epoch_num, batch_size, learning_rate, training_data, validation_data,
        ...
        with self.graph.as_default():
            ...
            for epoch_iter in tqdm(range(epoch_num)):
                ...
                if self.val_loss < max(best_epoch_losses): # (if top 5 performance on val:)
                    ...
                    checkpoint_path = (io.tf_save_path + "model_" +
        ...
        plt.savefig("%strain_loss_per_epoch.png" % io.tf_log_path)
        plt.close(1)
        ...
        plt.savefig("%sval_loss_per_epoch.png" % io.tf_log_path)
        plt.close(1)
    ...
    def graph_setup(self):
        ...
        with self.graph.as_default():
            ...
            #self.full_time_series = tf.placeholder(tf.float32, [None, self.output_size],name='full_time_series')
