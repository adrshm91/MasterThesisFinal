...
class Network(object):
        ...
        with self.graph.as_default():
            ...
            for epoch_iter in tqdm(range(epoch_num)):
                ...
                if self.val_loss < max(best_epoch_losses): # (if top 5 performance on val:)
                    ...
                    checkpoint_path = (self.io_nn.checkpoints_dir + "model_" +
    ...
    def save(self, file_name):
        ...
        with self.graph.as_default():
            ...
            saver.save(self.session, self.io_nn.checkpoints_dir + file_name + '.ckpt')
            ...
            with open(self.io_nn.checkpoints_dir + file_name + '.pkl', 'wb') as f:
    ...
    def from_saved(cls, file_name, io_nn, change_params={}):
        ...
        """
        with open(io_nn.checkpoints_dir + file_name + '.pkl', 'rb') as f:
    ...
    def load(self, file_name):
        ...
        with self.graph.as_default():
            saver = tf.train.Saver(tf.trainable_variables())
            saver.restore(self.session, self.io_nn.checkpoints_dir + file_name + '.ckpt')
